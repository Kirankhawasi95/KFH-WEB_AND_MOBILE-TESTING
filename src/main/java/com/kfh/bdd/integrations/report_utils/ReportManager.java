package com.kfh.bdd.integrations.report_utils;
...
//                                                   dateStamp + "/" + "kfh-" + timeStamp + ".html");
			...
			if (str_Execution_TYPE.equalsIgnoreCase("Web_UI")) {
				...
				spark.config().setDocumentTitle("kfh Web Report");
				spark.config().setReportName("kfh");

				extent.setSystemInfo("Organization", "kfh");
				...
				// spark.config().setEncoding("utf-8");
				spark.config().setTheme(Theme.DARK);
				spark.config().setDocumentTitle("kfh Mobile Report");
				spark.config().setReportName("kfh");
				...
				spark = new ExtentSparkReporter(Constants.extent_desktopreportPath);
				extent.attachReporter(spark);
				//spark.config().setEncoding("utf-8");
				spark.config().setTheme(Theme.DARK);
				spark.config().setDocumentTitle("kfh Desktop Report");
				spark.config().setReportName("kfh");
	 ...
	 * 
	 * ExtentTest test = extent.createTest(testName);
	 * test.assignCategory(categories); testMap.put(Thread.currentThread().getId(),
	 * test);
	 * 
	 * }
	 */
	...
	public static void startApiReport() {
		...
		if (htmlReporterAPI == null) {
			...
			extentAPI.setSystemInfo("Host Name", "kfh");
			extentAPI.setSystemInfo("Environment", "Stage");
			extentAPI.setSystemInfo("User Name", "Kriss");
			System.out.println();
			htmlReporterAPI.config().setDocumentTitle("kfh API Report");
			// Name of the report
			htmlReporterAPI.config().setReportName("kfh");
